...
def check_moves(file, prev_line, result):
    ...
    for line, row in enumerate(file[prev_line:]):
        ...
        # find if at the end of moves is the result
        if result in row:
        ...
        for i in range(index_of_dot + 2, len(move)):
            ...
            if is_comment == 0:
                if move[i] == ' ' or move[i] == '\n':
                    ...
                    if i+1 < len(move):
                        if move[i+1] == ' ':
                            raise_error(move, "bad move - too many spaces")
                    if is_submove(move[k:i]) == False:
                        raise_error(move, "bad move or result doesn't corelate with result tag")
                    k = i + 1

            if move[i-1] == '}':
                is_comment = 0

        # check if the moves aren't missing
        if half_move == 1:
            raise_error(-1, f"Expected more moves on move {n-1}")

        if space_counter < 2:
            half_move = 1
    else:
        if half_move == 0:
            raise_error(move, "bad move - too many half moves")
